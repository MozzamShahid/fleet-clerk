...
const UserProfile = () => {
    const { user } = useUser();  // Get the currently logged-in user's details
            ...
            const fetchProfileData = async () => {
                try {
                    ...
                    if (driverDoc.exists()) {
                        ...
                        console.log(`No driver details found for user ID: ${user.id}`);
                        toast.error("No driver details found.");
                    }

                    // Additional fetch logic for cars and maintenance can be added here as needed
...
function renderMaintenanceDetails(maintenanceDetails) {
    return (
        <table className="min-w-full table-auto text-sm text-left text-gray-500">
            ...
            <tbody>
                {maintenanceDetails.map((maintenance) => (
                    <tr key={maintenance.id} className="bg-white border-b">
